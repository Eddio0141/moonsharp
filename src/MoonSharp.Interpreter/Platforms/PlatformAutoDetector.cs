...
namespace MoonSharp.Interpreter.Platforms
{
        ...
        private static void AutoDetectPlatformFlags()
        {
	...
	#endif
#elif !(NETFX_CORE)
            // if Xunit.FactAttribute exists then we are running on a test runner
            var foundTest = AppDomain.CurrentDomain.GetAssemblies()
                .Any(a => a.SafeGetTypes().Any(t => t.FullName == "Xunit.FactAttribute"));
            if (foundTest)
            {
            ...
            else
            {
                IsRunningOnUnity = AppDomain.CurrentDomain
                    ...
                    .Any(t => t.FullName?.StartsWith("UnityEngine.") is true);
            }
#endif
#endif
